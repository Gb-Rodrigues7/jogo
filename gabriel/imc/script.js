...
let JogadorNome = '';
let circuloInterval; //Intervalo para criar circulos
...
function reiniciarJogo() {
    ...
    reiniciarBtn.style.display = 'none'; // Oculta pontuação final
...
function criarCirculo() {
    ...
    let cair = setInterval(() => {
        if (parseInt(circulo.style.top) < containerJogo.clientHeight - 20) { // Ajuste para não sair do contêiner
...
document.addEventListener('keydown', (event) => {
    ...
    break;
        case 'ArrowRight':
        case 'd':
            if (parseInt(jogador.style.left) < containerJogo.clientWidth - 30) { // Precine que saia do contêiner
                jogador.style.left = (parseInt(jogador.style.left) + 7) + 'px'; // Aumenta a velocidade
    }
    break;
        case ' ':
